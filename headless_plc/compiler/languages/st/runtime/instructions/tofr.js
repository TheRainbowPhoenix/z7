...
export function tofr(timer, scanTime) {
    if (typeof timer !== 'object' || timer == null) {
        throw new Error('TOFR expects a timer structure');
    }
    ...
    timer.Reset = reset;
    let accumulator = Math.max(0, Math.min(toDint(timer.ACC), preset));
    let instructFault = 0;
    let presetInv = 0;
    if (presetInvalid) {
        instructFault = 1;
        ...
        accumulator = Math.max(0, accumulator - scanTime);
    }
    const en = presetInvalid || reset ? 0 : enabled ? 1 : 0;
    const timing = presetInvalid || reset ? 0 : !enabled && accumulator > 0 ? 1 : 0;
    const done = presetInvalid
        ? 0
        : enabled
            ? 1
            : accumulator > 0
