...
class Client:
    ...
    def _tcp_send(self, send_data:bytes):
        ...
        if self._tcp_connected:
            ...
            print(f">>> TCP SEND ({len(send_data)}): {send_data.hex()}")
            try:
    ...
    def _iso_send(self, send_data:bytes):
        ...
        if self._tcp_connected and self._iso_connected:
            ...
            print(f">>> ISO SEND ({len(send_data)}): {send_data.hex()}")
            try:
                self._sock.send(send_data)
            except socket.timeout:
    ...
    def _send(self, send_data:bytes):
        ...
        if self._tcp_connected and self._iso_connected and self._pdu_negotiated:
            ...
            print(f">>> SEND ({len(send_data)}): {send_data.hex()}")
            try:
                self._sock.send(send_data)
            except socket.timeout:
    ...
    def _recv(self):
        ...
        try:
            ...
            print(f"<<< RECV ({len(data)}): {data.hex()}")
        except socket.timeout:
    ...
    def read_cpu_info(self) -> CPUInfo:
        ...
        print(f"DEBUG: read_cpu_info returning systemName='{cpuInfo.systemName}'")
        return cpuInfo
    ...
    def read_block_info(self, block_type:int, block_number:int) -> list:
        ...
        if (length > 32): # the minimum expected
            ...
            if (param_error_code == const.ParamErrorCode.NO_ERROR 
                ...
                # update cache
                bt_str = blockInfo.type
                bn = blockInfo.number
                if self.cache_data.get(bt_str) is None:
                    self.cache_data[bt_str] = {}   
                self.cache_data[bt_str][bn] = blockInfo
                print(f"DEBUG: read_block_info updated cache for {bt_str}{bn}. mc7Length={blockInfo.mc7Length}")
            else:
    ...
    def read_area_raw(self, address:str, elements:int) -> list:
        ...
        if (self.controller < 1200):
            try:
                ...
                print(f"DEBUG: mc7_length for {area_type[0]}{db_number} is {mc7_length}")
            except Exception as e:
                print(f"DEBUG: read_area_raw exception: {e}")
                mc7_length = elements
                pass
    ...
    def read_area(self, item_list:list=[]) -> list:
        ...
        while total_items_index < total_items:
            ...
            if item.type == const.DataType.STRING and not skip:
                ...
                # parse buffer
                if len(response[0].value) < 128:
                    print(f"WARNING: String read for {item.address} returned only {len(response[0].value)} bytes, expected 128. Padding with zeros.")
                    response[0] = response[0]._replace(value=response[0].value.ljust(128, b'\x00'))
                (
                    max_length, 
                    string_length, 
                    string_value
