...
class Test_WString_Split_Multiple_Clean_Output(unittest.TestCase):
    def setUp(self):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.WString_Split_Multiple_Clean_Output as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'Array_Any_Length')

        context = DotDict({
            'array': RecursiveMock(f'array'),
            'status': 0,
            'ret': 0,
        })
        ...
        try:
            ...
            for _, name, _ in pkgutil.iter_modules(pkg.__path__):
                if name == 'WString_Split_Multiple_Clean_Output': continue
                try:
