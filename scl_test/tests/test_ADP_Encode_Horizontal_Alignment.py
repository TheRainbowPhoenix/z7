...
class Test_ADP_Encode_Horizontal_Alignment(unittest.TestCase):
    def setUp(self):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.ADP_Encode_Horizontal_Alignment as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'ADP_Encode_Horizontal_Alignment')

        context = DotDict({
            'horizontal_alignment': '',
            'decoded': RecursiveMock(f'decoded'),
            'encoded': RecursiveMock(f'encoded'),
            '_return': '',
        })
        ...
        try:
            ...
            for _, name, _ in pkgutil.iter_modules(pkg.__path__):
                if name == 'ADP_Encode_Horizontal_Alignment': continue
                try:
