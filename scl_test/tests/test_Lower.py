...
class Test_Lower(unittest.TestCase):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.Lower as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'Lower')

        context = DotDict({
            'string': '',
            'charArr': RecursiveMock(f'charArr'),
            'cntChar': 0,
            'pChar': 0,
            'stringBox': '',
            'asciiLow': RecursiveMock(f'asciiLow'),
            'asciiHigh': RecursiveMock(f'asciiHigh'),
            'asciiCaseOffset': 0,
        })
