...
class Test_MQTT_Runtime(unittest.TestCase):
    def setUp(self):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.MQTT_Runtime as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'mqttRuntime')

        context = DotDict({
            'IN': False,
            'RES': False,
            'RTIME': RecursiveMock(f'RTIME'),
            'last': 0,
            'now': 0,
            'run': 0,
            'FP_IN': False,
