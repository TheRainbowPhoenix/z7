...
class Test_Unixtime(unittest.TestCase):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.Unixtime as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'Unixtime')

        context = DotDict({
            'leapyears': 0,
            'days_from_1970': 0,
            'days_from_BeginOfYear': 0,
            'l_unixtime': 0,
            'sfc1_ret_val': 0,
            'CDT_systemtime': RecursiveMock(f'CDT_systemtime'),
            'systemtime': RecursiveMock(f'systemtime'),
            'year': 0,
            'month': 0,
            'day': 0,
            'hour': 0,
