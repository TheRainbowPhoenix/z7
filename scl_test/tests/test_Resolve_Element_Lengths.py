...
class Test_Resolve_Element_Lengths(unittest.TestCase):
    def setUp(self):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.Resolve_Element_Lengths as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'Resolve_Element_Lengths')

        context = DotDict({
            'formatCodes': '',
            'elementLengths': RecursiveMock(f'elementLengths'),
            'error message': '',
            'code': '',
            'pCode': 0,
            'errorFlag': False,
        ...
        try:
            ...
            for _, name, _ in pkgutil.iter_modules(pkg.__path__):
                if name == 'Resolve_Element_Lengths': continue
                try:
