...
class Test_WString_to_Time(unittest.TestCase):
    def setUp(self):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.WString_to_Time as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'WString_to_Time')
        ...
        context = DotDict({
            ...
            'DInt Box': 0,
            'String Box': '',
            'Time Box': RecursiveMock(f'Time Box'),
            'WChar Array': RecursiveMock(f'WChar Array'),
            'ms/(10 hr)': 0,
            'ms/hr': 0,
            'ms/(10 min)': 0,
            'ms/min': 0,
        ...
        try:
            ...
            for _, name, _ in pkgutil.iter_modules(pkg.__path__):
                if name == 'WString_to_Time': continue
                try:
