...
class Test_Linked_List(unittest.TestCase):
    ...
    def test_run(self):
        ...
        import scl_test.transpiled.Linked_List as transpiled_module

        # We need to find the function in the module
        func = getattr(transpiled_module, 'Linked_List__Private__Clear_Node')

        context = DotDict({
            'node': RecursiveMock(f'node'),
            'ret': RecursiveMock(f'ret'),
        })
