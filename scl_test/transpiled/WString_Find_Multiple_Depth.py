import math

def Array_Any_Length(context=None, global_dbs=None, **kwargs):
    if context is None: context = {}
    if global_dbs is None: global_dbs = {}
    for k, v in kwargs.items():
        context[k] = v
    # Return Type: UDInt
    # VAR_INPUT:
    #   array: Variant
    # VAR_TEMP:
    #   status: DInt
    #   ret: UDInt
    context['ret'] = CountOfElements(context['array'])
    context['Array.Any.Length'] = context['ret']


def WString_Find_Multiple_Depth(context=None, global_dbs=None, **kwargs):
    if context is None: context = {}
    if global_dbs is None: global_dbs = {}
    for k, v in kwargs.items():
        context[k] = v
    # Return Type: Int
    # VAR_INPUT:
    #   body: WString
    #   terms: Array[*] of WString
    #   p_terms: Int
    # VAR_TEMP:
    #   _length: UDInt
    #   _position: Int
    # VAR CONSTANT:
    #   BLANK: WString
    context['_length'] = Array_Any_Length(context['terms'])
    if ((context['p_terms'] > context['_length']) or (context['terms'][int(context['p_terms'])] == context['BLANK'])):
        context['WString.Find_Multiple.Depth'] = 0
        return
    context['_position'] = FIND(IN1=context['body'], IN2=context['terms'][int(context['p_terms'])])
    if (context['_position'] == 0):
        context['_position'] = WString_Find_Multiple_Depth(body=context['body'], terms=context['terms'], p_terms=(context['p_terms'] + 1))
    context['WString.Find_Multiple.Depth'] = context['_position']
