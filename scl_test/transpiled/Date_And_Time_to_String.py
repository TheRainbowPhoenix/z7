...
def Date_And_Time_to_String(context=None, global_dbs=None, **kwargs):
    if context is None: context = {}
    if global_dbs is None: global_dbs = {}
    for k, v in kwargs.items():
        context[k] = v
    ...
    if (context['retError'] != ''):
        context['error message'] = context['retError']
        return
    Elements_to_Strings(date__and__time=context['date & time'], formatCodes=context['formatCodes'], date__and__time_as_string=context['d&tStrings'], error_message=context['retError'])
    if (context['retError'] != ''):
        context['error message'] = context['retError']
        return
    Format_Output(formatCodes=context['formatCodes'], delimeters=context['delimeters'], d_and_tStrings=context['d&tStrings'], date__and__time_as_string=context['retD&T'], error_message=context['retError'])
    context['date & time as string'] = context['retD&T']
