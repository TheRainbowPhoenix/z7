...
class Sequence_Data_Management(BaseBlock):
    ...
    def network_1(self):
        """Network 1"""
        if (True and self.DB_Equipment.SupplyStation.Command.xSupplyDataRequest):
            if not isinstance(self.UNKNOWN, DownloadNewData): self.UNKNOWN = DownloadNewData()
    ...
    def network_2(self):
        """Network 2"""
        if True:
            if not isinstance(self.UNKNOWN, PushDataToStation): self.UNKNOWN = PushDataToStation()
            self.UNKNOWN.Work_Station_GD = self.DB_Equipment.Station1
            self.UNKNOWN.WorkStationList = self.WorkStationList1
            self.UNKNOWN.run()
        if True:
            if not isinstance(self.UNKNOWN, PushDataToStation): self.UNKNOWN = PushDataToStation()
            self.UNKNOWN.Work_Station_GD = self.DB_Equipment.Station2
            self.UNKNOWN.WorkStationList = self.WorkStationList2
            self.UNKNOWN.run()
        if True:
            if not isinstance(self.UNKNOWN, PushDataToStation): self.UNKNOWN = PushDataToStation()
            self.UNKNOWN.Work_Station_GD = self.DB_Equipment.Station3
