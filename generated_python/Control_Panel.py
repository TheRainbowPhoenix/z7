...
def network_1(self):
    """Network 1"""
    self.xProcessLight = (True and self.Work_Station_DB_Status_xInProcess)

def network_2(self):
    """Network 2"""
    if False:
        if not hasattr(self, 'UNKNOWN'): self.UNKNOWN = TimeLeftCalcuation()
        self.UNKNOWN.diProcessTime_ms = self.diProcessTime
        self.UNKNOWN.diTimeDone_ms = self.Work_Station_DB_Status_diProcessTimeDone
