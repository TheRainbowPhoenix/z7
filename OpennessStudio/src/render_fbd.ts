...
const dbNumberCache = new Map<string, number | null>();
...
export async function renderFbd(filePath: string): Promise<string> {
  ...
  await enrichCallInstanceDbNumbers(node);
  ...
  htmlBody += renderInterfaceTable(node.interface);
    ...
    htmlBody += `
        <div class="network-container">
            <div class="network-header">
                <span class="network-title">${titleText}</span>
            </div>
            ${commentText}
  ...
  const styles = `
    <style>
        ...
        .network-comment { padding: 6px 10px; color: #d0d0d0; font-size: 12px; border-bottom: 1px solid #3a3a3a; background: #2b2b2b; }
        .iface-table { width: 100%; border-collapse: collapse; margin-bottom: 14px; background: #202022; border: 1px solid #3a3a3a; }
        .iface-table th, .iface-table td { border: 1px solid #3a3a3a; padding: 4px 6px; font-size: 12px; text-align: left; }
...
function layoutAndRenderNetwork(network: Network, netIndex: number): string {
  ...
  network.wires.forEach((w) => {
    ...
    const operandPin = points.find((p) =>
      p.type === "pin" && p.pinName === "operand"
    );
    if (operandPin) {
    ...
    if (accessPoint && !hasPowerrail) {
      ...
      const blockPoints = points.filter((p) =>
        p !== accessPoint && p.type === "pin"
      );
      ...
      if (blockPoints.length > 0) {
        ...
        if (accessNode) {
          ...
          blockPoints.forEach((bp) => {
            ...
            if (blockNode) {
              const pin = [...blockNode.inputs, ...blockNode.outputs].find(
                (p) => p.name === bp.pinName,
              );
              if (pin) {
    ...
    points.forEach((p) => {
      ...
      if (!node) return;
      if (
        source && p.nodeId === source.nodeId &&
        (p.pinName || "main") === source.pinName
      ) return;
  ...
  nodes.forEach((node) => {
    if (BASIC_BLOCKS.includes(node.data.name || "")) {
      node.inputs = node.inputs.filter((p) => {
        if (p.name === "EN" && !p.isConnected && !p.connectedVariable) {
          return false;
        }
        return true;
      });
      node.outputs = node.outputs.filter((p) => {
        if (p.name === "ENO" && !p.isConnected && !p.connectedVariable) {
          return false;
        }
        return true;
      });
  ...
  wires.forEach((wire) => {
    ...
    if (wire.source) {
      if (wire.source.nodeId === "Powerrail") {
        ...
        if (firstTgt) {
          const tgtPin = firstTgt.inputs.find((p) =>
            p.name === wire.targets[0].pinName
          ) || { relY: 10 };
        ...
        if (srcNode) {
          ...
          const srcPin = srcNode.outputs.find((p) =>
            p.name === wire.source!.pinName
          ) || { relY: srcNode.height / 2, relX: 0 };
      ...
      wire.targets.forEach((tgt) => {
        ...
        if (tgtNode) {
          let tgtX = tgtNode.x;
          const tgtPin = tgtNode.inputs.find((p) => p.name === tgt.pinName) ||
            { relY: tgtNode.height / 2, relX: 0 };
          ...
          svg +=
            `<path d="M ${srcX} ${srcY} C ${c1x} ${c1y}, ${c2x} ${c2y}, ${tgtX} ${tgtY}" class="wire" />`;
        }
      });
    }
  });
...
function getPins(part: NetworkPart, type: "input" | "output"): GraphPin[] {
  const pins: GraphPin[] = [];
  const isNegated = (name: string) =>
    part.negated && part.negated.includes(name);
  ...
  if (part.type === "Part" || part.type === "Call") {
    if (part.type === "Call" && part.callInfo) {
      ...
      part.callInfo.parameters.forEach((p) => {
        if (
          type === "input" && (p.section === "Input" || p.section === "InOut")
        ) {
          pins.push({
            name: p.name,
            type: "input",
            relY: 0,
            isNegated: isNegated(p.name),
          });
        }
        if (
          type === "output" && (p.section === "Output" || p.section === "InOut")
        ) {
          pins.push({
            name: p.name,
            type: "output",
      ...
      if (type === "input") {
        ...
        // Logic Gates
        if (
          ["A", "O", "X", "AND", "OR", "XOR"].includes(name) || name === "&" ||
          name === ">=1"
        ) {
...
function calculateNodeWidth(
  ...
  if (part.type === "Access") {
    const len = part.symbol?.components.map((c) => c.name).join(".").length ||
      10;
  ...
  return Math.max(
    BLOCK_MIN_WIDTH,
    Math.min(Math.max(titleLen, maxRowW), BLOCK_MAX_WIDTH),
  );
}
...
function renderPins(pins: GraphPin[], width: number) {
  return pins.map((p) => {
    ...
    let decoration = "";
    if (p.isNegated) {
      decoration = `<circle cx="${
        isInput ? 0 : width
      }" cy="${p.relY}" r="3" class="pin-negated" />`;
    }
    ...
    if (p.connectedVariable) {
      ...
      const labelWidth = Math.max(
        VAR_BOX_MIN_WIDTH,
        Math.min(rawW, VAR_BOX_MAX_WIDTH),
      );
      ...
      labelSvg = `<g>
                ...
                <rect x="${boxX}" y="${
        p.relY - 10
      }" width="${labelWidth}" height="20" class="variable-box" />
                <text x="${textX}" y="${
        p.relY + 4
      }" class="variable-text" text-anchor="${anchor}">${dispText}</text>
             </g>`;
    }

    // Always draw the tick line
    const lineSvg =
      `<line x1="${xLineStart}" y1="${p.relY}" x2="${xLineEnd}" y2="${p.relY}" stroke="black" />`;
...
function renderOperandLabel(node: GraphNode, width: number) {
  ...
  return `<g transform="translate(0, -22)">
        ...
        <text x="${
    width / 2
  }" y="14" text-anchor="middle" font-weight="bold" fill="black">${node.operandLabel}</text>
    </g>`;
}
...
function renderInterfaceTable(blockInterface?: BlockInterface): string {
  ...
  for (const section of blockInterface.sections) {
    rows.push(
      `<tr class="iface-section-row"><td colspan="4">${
  ...
  return `<table class="iface-table">
        <thead>
            <tr><th>Name</th><th>Data type</th><th>Default value</th><th>Comment</th></tr>
        </thead>
...
function formatCallTypeLabel(part: NetworkPart): string {
  if (part.type !== "Call") return "";
  const blockType = part.callInfo?.blockType
    ? `%${part.callInfo.blockType}`
    : "";
  const instanceName = part.callInfo?.instanceName;
  if (!instanceName) return blockType;
  const dbSuffix = part.callInfo?.instanceDbNumber
    ? ` DB${part.callInfo.instanceDbNumber}`
    : "";
  return `${blockType} ${instanceName}${dbSuffix}`.trim();
}

async function enrichCallInstanceDbNumbers(node: OpennessNode): Promise<void> {
  if (!node.networks?.length) return;

  for (const network of node.networks) {
    for (const part of network.parts) {
      if (part.type !== "Call" || !part.callInfo?.instanceName) continue;
      const dbNumber = await resolveDbNumberForInstance(
        part.callInfo.instanceName,
      );
      if (dbNumber !== null) part.callInfo.instanceDbNumber = dbNumber;
    }
  }
}

async function resolveDbNumberForInstance(
  instanceName: string,
): Promise<number | null> {
  if (dbNumberCache.has(instanceName)) {
    return dbNumberCache.get(instanceName) ?? null;
  }

  const candidatePath = fileMap[instanceName];
  if (!candidatePath) {
    dbNumberCache.set(instanceName, null);
    return null;
  }

  try {
    const parsed = await parseFile(candidatePath);
    const number = parsed?.type === BlockType.DB ? parsed.number ?? null : null;
    dbNumberCache.set(instanceName, number);
    return number;
  } catch {
    dbNumberCache.set(instanceName, null);
    return null;
